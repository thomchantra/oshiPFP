...
interface BlankStateProps {
  ...
  circle: boolean
  /** Opens AlgoGalleryModal (v0.3 polish pass) — lets someone browse the algorithm/preset gallery
   * and pick a "Load Demo" before ever uploading their own photo. */
...
export default function BlankState({ onLoadFile, circle, onBrowseGallery }: BlankStateProps) {
  ...
  return (
    <div className="blank-state-area">
      {/* Drag handlers live on this plain div (not a <label>, per v0.3 polish pass) so "Browse
          Gallery" below can be a normal sibling button — a button nested inside a <label>
          associated with the file input risked also triggering the file picker on click. The
          <label> below is scoped tightly to just the icon/text/input, the actual click-to-browse-
          files affordance. */}
      <div
