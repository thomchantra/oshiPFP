/**
 * Shared GLSL for the "what color goes here" decision used by Fill mode (fillMask.frag.ts) and
 * Line mode (blobMask.frag.ts, via lineFillColor.frag.ts) — factored out to avoid duplicating
 * this Image/Solid/Gradient choice and its 3-stop luminance recolor math across algorithms.
 *
 * Every call site that binds this program must set every uniform below explicitly on every
 * call — see CLAUDE.md's "Known Recurring Gotchas" on shared GL program uniforms leaking state
 * across call sites.
 *
 ...
 *
 * uVividBoost/uVividDeadzone: optional saturation-boost modulation of the 'image' branch's
 * passthrough color via an HSV round-trip. Neutral at uVividBoost=1 (any uVividDeadzone) — the
 * round-trip is a no-op there, so callers that don't use vivid boost can just set uVividBoost=1
 * rather than branching around it.
 *
 * uColorContrast: a *global* Image-subtab parameter — a pivoted-at-0.5 contrast curve, same
 * formula distanceToEdge.frag.ts applies to its own fused image path. Neutral at
...
// Gradient stop positions from uGradientPivot. uGradientPivot is negated before use so that -1
// visually leans the image toward shadow, +1 toward highlight. Internally, at pivot=0 stops sit
// at luminance 0 (shadow) / 0.5 (mid) / 1 (highlight); internal pivot<0 compresses the
// mid+highlight stops toward 0 (shadow stays pinned) — what a slider value of +1 (negated to
// internal -1) visually produces (image reads highlight-dominated); slider -1 (internal +1)
// compresses shadow+mid toward 1, reading shadow-dominated. Duo Tone drops the mid stop, leaving
// only shadow/highlight — pivot then only compresses whichever end it leans toward.
