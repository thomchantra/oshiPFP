 ...
 *
 * Built for Daiya's soft-threshold feature (pipeline.ts's pathD branch): JFA's distance
 * transform needs a strictly binary seed mask, so a soft/antialiased threshold can't replace it
 * directly — but multiplying a separately-computed soft threshold weight into the
 ...
 *
 * uMaskGain — the raw smoothstep mask reads as faint/washed-out at any real softness width (most
 * of the transition band sits well under full alpha), so this boosts it
