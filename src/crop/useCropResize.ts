 ...
 * switching to another tab and back. Unlike useExportSettings (whose state
 * is read directly by ExportPanel, no pipeline side effect), Resize actually
 * changes the working pipeline resolution, so this hook pushes changes into
 * the pipeline via onResizeChange.
 *
 * cropSize seeds the initial custom-size default the same way
 * useExportSettings(cropSize) does. Note cropSize here is Pipeline.cropSize,
 * which reflects the *resized* output, not the raw crop — but while
 * resizeMode is 'original' the resize stage is aliased straight to
 * cropTarget (see pipeline.ts), so cropSize still equals the raw crop's
 * native size in that state, which is exactly when this default/prefill
 * logic runs.
 */
export function useCropResize(
  ...
  // Back to this hook's own IDENTITY_RESIZE shape.
  const reset = () => {
